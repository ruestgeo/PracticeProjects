...
public class Player extends Actor {
    ...
    */
    ...
    public Player (String id, String name, String session, GameRoom room){
        super(room,-1,-1);
        this.id = id;
        this.name = name; 
    ...
    public void sendAsync (String message){
        if (message == null)
            return;
        SessionManager.get("ww").send(this.session, message);
    }
    ...
    public void sendSync (String message){
        if (message == null)
            return;
        try{
            SessionManager.get("ww").sendSync(this.session, message);
        }
        catch (IOException e){
