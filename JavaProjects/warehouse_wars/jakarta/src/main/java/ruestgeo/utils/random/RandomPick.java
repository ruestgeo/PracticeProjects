...
public class RandomPick<T> {
     ...
     */
    public void set (T item, int weight){
        ready = false;
     ...
     */
    public void add (T item, int weight) throws IllegalAccessError{
        if ( this.items.contains(item) )
     ...
     */
    public void remove (T item, int weight) throws IllegalStateException, IllegalAccessError{
        ready = false;
        //if not in set, throw error
        if ( !this.items.contains(item) )
    ...
    public void calculateProbabilities (){
        var totalWeight = this.weights.stream().reduce(0, (a,b) -> a + b); //Integer::sum
